...
function loginErrorMessage(reason: unknown): string {
  if (reason instanceof ApiError && reason.code === "AUTH_FAILED") {
    return "账号或密码不正确，或当前账号暂时无法登录。";
  }
  if (reason instanceof ContractError) {
    return "登录服务返回异常，请稍后重试。账号和密码尚未完成校验。";
  }
  if (reason instanceof TypeError || (reason instanceof ApiError && reason.status >= 500)) {
    return "暂时无法连接登录服务，请稍后重试。账号和密码尚未完成校验。";
  }
    ...
    <main className="login-page">
      ...
      <section className="login-form-wrap">
        <form className="login-form" onSubmit={(event) => void submit(event)} noValidate>
          ...
          {error ? <div className="notice notice--error" role="alert"><span className="notice__icon" aria-hidden="true">!</span><p>{error}</p></div> : null}
